...
CONFIG_FILES = ['/root/MyAnsibe/openstack.yaml']
...
### Do not need anything related to cloud because I use only one cloud. The script is intended to be used  on more cloud instances
   ...
   # groups.append(cloud)
    ...
    vstate = server_vars.get('vm_state', None)
    if vstate:
        # Make groups for virtual machine state
        groups.append(vstate)
    ...
    try:
        ...
        if hasattr(sdk_inventory.OpenStackInventory, 'extra_config'):
            inventory_args.update(dict(
                config_key='ansible',
                config_defaults={
                    'use_hostnames': True, #False, (If i leave this as False the quesry will display instance ID rather than Name)
